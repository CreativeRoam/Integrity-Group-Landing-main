<% if (vert) { %>
    ...
    in vec4 aWeights_0;         // 4 weights per vertex
    in vec4 aBoneNdx_0;         // 4 bone indices per vertex

    <%if (defines["ATTRIB_STACK"] >= 2) { %>
        in vec4 aWeights_1;         // 4 weights per vertex
        in vec4 aBoneNdx_1;         // 4 bone indices per vertex
    <% } %>
    <%if (defines["ATTRIB_STACK"] >= 3) { %>
        in vec4 aWeights_2;         // 4 weights per vertex
        in vec4 aBoneNdx_2;         // 4 bone indices per vertex
    <% } %>
    ...
    void main(){
        ...
        vPosition = vec3(pos.xyz) / pos.w;
            ...
            #else // HAS_TANGENTS != 1
                vNormal = normalize(vec3(normalMatrix * skinedNorm.xyz));
...
<% if (frag) { %>
    ...
    void main() {   
        ...
        #endif
        // outColor = vec4(SampleSphericalMap(reflection).x, 0., 0., baseColor.a);

        // #ifdef HAS_NORMALMAP
        //     outColor.rgb = vec3(texture(uNormalSampler, vUv).rgb);
        //     outColor.a = 1.;
        //     // outColor.rgb = vec3(texture(uNormalSampler, vUv).rgb);
        // #endif
