...
const _VEC31 = vec3.create()
const _VEC32 = vec3.create()
const _QUAT1 = quat.create()
const _QUAT2 = quat.create()
const _QUAT3 = quat.create()
...
class Animator {
  constructor(scene, nodeTree, rootNode, mesh) {
    ...
    this.blendDamping       = 0.02

    this.positionOffset     = { x: 0, y: 0, z: 0, rx: 0, ry: 0, rz: 0}
    this.states             = {}

    this.nodesDictionary    = this.nodeTree
  }
  ...
  applyAnimationFromTracks(id1, id2, blend) {
    ...
    for (let i = 0; i < poses1.length; i++) {
      const n1 = poses1[i]
      if (this.nodesDictionary[n1.name] == undefined) {
        continue
      }
      const fNode = this.nodesDictionary[n1.name].transform
      ...
      quat.slerp(_QUAT3, _QUAT1, _QUAT2, blend)

      if (fNode.offsetRotation) {
        quat.rotateY(_QUAT3, _QUAT3, fNode.offsetRotation.y)
        quat.rotateX(_QUAT3, _QUAT3, fNode.offsetRotation.x)
        quat.rotateZ(_QUAT3, _QUAT3, fNode.offsetRotation.z)
      }

      fNode.quaternion.x = _QUAT3[0]
      fNode.quaternion.y = _QUAT3[1]
      fNode.quaternion.z = _QUAT3[2]
      fNode.quaternion.w = _QUAT3[3]

      fNode.readyToUse = true

      if (n1.name == this.rootNode.name) {
        // console.log(n1.rotation);
        if (id2 !== null) {
          vec3.lerp(fNode.position, n1.position, poses2[i].position, blend)
        } else {
          vec3.copy(fNode.position, n1.position)
        }
        vec3.add(fNode.position, fNode.position, fNode.offsetPosition)
        vec3.scale(fNode.position, fNode.position, 0.025)
      } else {
        // vec3.copy(fNode.position, n1.position)
        fNode.updateMatrix()
  ...
  update() {
    this.blendDamping = 15 * (this.scene.dt / 1000)
