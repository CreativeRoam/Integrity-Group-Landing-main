...
class SceneSubAlpha extends SubsceneAbs {
  ...
  initDummy() {
    ...
    // Test sphere
    const sphereGeom = new Sphere(this.gl)
    const sphereShader = new Shader(ShaderManifest['debugUnlit'])
    const sphereProgram = new Program(this.gl, {
      vertex: sphereShader.vert,
      fragment: sphereShader.frag,
      uniforms: {
        uColor: { value: new Color('#FF0000') },
        uAlpha: { value: 1 },
      },
    })
    this.sphere = new Mesh(this.gl, { geometry: sphereGeom, program: sphereProgram })
    this.sphere.position.y = 1
    this.sphere.setParent(this.root)
  }
  ...
  preRender() {
    ...
    this.sphere.position.y = 1.0 + Math.cos(this.time * 2) * 0.2
    this.sphere.rotation.x = this.time
    this.sphere.rotation.y = this.time
