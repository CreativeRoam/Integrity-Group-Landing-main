...
BinaryMeshLoader.prototype = {
  init: function () {
    ...
    var defaultProperties = ['vertices', 'normals', 'uvs', 'indices', 'vertexIndex', 'name', 'type', 'translate', 'scale', 'rotation', 'infos', 'material', 'maxJoints', 'attribStackNum', 'joints', 'sid', 'parent']
    ...
    for (var i = 0; i < objects.length; i++) {
      ...
      if (el.normal != null) {
        var v = el.normal
        el.normals = new Float32Array(this.bin.slice(v.offset * 4, v.offset * 4 + v.length * 4))
      }
      if (el.uvs != null) {
        var v = el.uvs
      ...
      if (el.vertexIndex != null) {
        var v = el.vertexIndex
        var index = new Float32Array(this.bin.slice(v.offset * 4, v.offset * 4 + v.length * 4))
        var tmp = []
        for (var j = 0; j < index.length; j++) {
          tmp.push(index[j])
        }
        el.vertexIndex = new Uint16Array(tmp)
      }
...
export default BinaryMeshLoader
